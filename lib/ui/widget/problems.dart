...
class _ProblemsState extends State<Problems> {
  ...
  //问题类型列表
  final List<String> problemTypes = ['功能建议', '操作体验', '卡顿/闪退', '其他反馈'];
          ...
          child: Column(
            ...
            children: [
              ...
              Row(
                ...
                children: [SubmitButton(onPressed: _submitFeedback)],
              ),
            ],
          ),
        ),
  ...
  Future<void> _submitFeedback() async {
    if (_selectedProblemType == null) {
      ScaffoldMessenger.of(
        context,
      ).showSnackBar(SnackBar(content: Text('请选择问题类型')));
      return;
    }
    if (_detailController.text.isEmpty) {
      ScaffoldMessenger.of(
        context,
      ).showSnackBar(SnackBar(content: Text('请描述具体问题')));
      return;
    }
    if (_contactController.text.isEmpty) {
      ScaffoldMessenger.of(
        context,
      ).showSnackBar(SnackBar(content: Text('请输入联系方式')));
      return;
    }

    try {
      //准备请求参数
      String content = _detailController.text;
      String contact = _contactController.text;
      //图片暂时设置为"无"，实际项目中需要实现图片上传
      String image = "无";

      //发送网络请求
      await _apiService.submitFeedback(content, contact, image);
